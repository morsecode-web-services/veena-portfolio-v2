...
async function optimizeImage(filePath) {
  ...
  try {
    ...
    if (buffer.length < originalSize) {
      ...
      console.log(
        `  ✓ ${path.relative(IMAGES_DIR, filePath)}: ${formatBytes(originalSize)} → ${formatBytes(buffer.length)} (-${savings}%)`
      );
    } else {
      console.log(
        `  ○ ${path.relative(IMAGES_DIR, filePath)}: already optimal (${formatBytes(originalSize)})`
      );
    }
  } catch (err) {
