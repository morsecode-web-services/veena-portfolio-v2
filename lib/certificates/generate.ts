...
export async function generateAndUploadCertificate(enrollmentId: string): Promise<string> {
  ...
  const rawOutcomes =
    'Raga exploration & ear training|Tala rhythm mastery|Live performance practice'; // Fallback or dynamic logic can go here
  ...
  // 5. Generate PNG
  const response = new ImageResponse(createElement(ParticipationCertificateImage, props), {
    width: 800,
    height: 1000,
    fonts: [
