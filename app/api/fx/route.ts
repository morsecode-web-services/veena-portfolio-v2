...
export async function GET(request: NextRequest) {
  ...
  if (targetCurrency) {
    ...
    if (targetCurrency === 'INR') {
      return NextResponse.json({ rate: 1, currency: 'INR', symbol: '₹', base: 'INR' });
    }
    ...
    const config = countryCurrencyMap[ipCountry] || countryCurrencyMap['US'];
  ...
  try {
    ...
    return NextResponse.json({ rate, currency: targetCurrency, symbol: targetSymbol, base: 'INR' });
  } catch (error) {
