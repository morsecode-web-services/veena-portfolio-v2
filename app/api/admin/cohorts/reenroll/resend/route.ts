...
export async function POST(request: Request) {
  try {
    ...
    if (invitation.status !== 'wa_failed') {
      return NextResponse.json(
        {
          error: `Cannot resend: invitation status is '${invitation.status}', expected 'wa_failed'`,
        },
        { status: 400 }
      );
    }
    ...
    // Update the invitation record: reset to 'sent' with new message SID and pending delivery status
    ...
    if (updateErr) {
      console.error('[Resend API] Failed to update invitation after resend:', updateErr);
      // Still return success — the message was sent
    }

    return NextResponse.json({ success: true, messageSid: waRes.messageSid });
  } catch (error: any) {
