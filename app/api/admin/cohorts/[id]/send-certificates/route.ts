...
export async function POST(
    ...
    try {
        ...
        if (templateError || !template) {
            return NextResponse.json({ error: 'Certificate template not configured for this cohort. Please build a template first.' }, { status: 400 });
        }
        ...
        for (const enrollment of enrollments || []) {
            try {
                ...
                const twilioRes = await sendTwilioWhatsApp(student.phone, message, contentSid, contentVariables);
