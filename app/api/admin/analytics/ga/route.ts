...
export async function GET(request: Request) {
  try {
    ...
    // Parse Funnel
    const funnel = funnelReport.rows?.reduce(
      (acc: any, row: any) => {
        acc[row.dimensionValues[0].value] = parseInt(row.metricValues[0].value, 10);
        return acc;
      },
      { view_item: 0, begin_checkout: 0, purchase: 0 }
    ) || { view_item: 0, begin_checkout: 0, purchase: 0 };
    ...
    // Parse countries
    const countries =
      countriesReport.rows?.map((row: any) => ({
    ...
    // Parse devices
    const devices =
      devicesReport.rows?.map((row: any) => ({
    ...
    const overview = {
      users: {
        value: getVal(currentMetrics, 0),
        change: getPct(getVal(currentMetrics, 0), getVal(previousMetrics, 0)),
      },
      pageViews: {
        value: getVal(currentMetrics, 1),
        change: getPct(getVal(currentMetrics, 1), getVal(previousMetrics, 1)),
      },
      sessions: {
        value: getVal(currentMetrics, 2),
        change: getPct(getVal(currentMetrics, 2), getVal(previousMetrics, 2)),
      },
      avgDuration: {
        value: Math.round(getVal(currentMetrics, 3)),
        change: getPct(getVal(currentMetrics, 3), getVal(previousMetrics, 3)),
      },
      bounceRate: {
        value: Math.round(getVal(currentMetrics, 4) * 100),
        change: getPct(getVal(currentMetrics, 4), getVal(previousMetrics, 4)),
      },
      newUsers: {
        value: getVal(currentMetrics, 5),
        change: getPct(getVal(currentMetrics, 5), getVal(previousMetrics, 5)),
      },
    };
    ...
    const heatmap =
      heatmapReport.rows?.map((row: any) => ({
    ...
    // Parse browsers
    const browsers =
      browsersReport.rows?.map((row: any) => ({
    ...
    // Parse landing pages
    const landingPages =
      landingReport.rows?.map((row: any) => ({
