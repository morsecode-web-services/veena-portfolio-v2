...
export default function DynamicStandaloneFormPage({ params }: { params: Promise<{ slug: string }> }) {
      ...
      <div className="min-h-screen bg-navy-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-navy-600 leading-relaxed mb-8">
            Thank you for your interest! Registrations for this event are currently closed. Please check back later for future events.
          </p>
          <a href="/" className="inline-block px-8 py-4 bg-navy-900 text-white rounded-xl font-bold tracking-wide hover:bg-navy-800 transition-colors">
            Return to Homepage
          </a>
        </m.div>
      </div>
          ...
          >
            <div className="bg-navy-900 p-8 rounded-3xl text-white shadow-premium-xl relative overflow-hidden">
              ...
              <p className="text-navy-200 text-sm leading-relaxed mb-6">
                {formSettings?.description || "Please complete your payment before filling the form."}
              </p>
              ...
              <div className="bg-navy-800/50 rounded-2xl p-4 mb-6 backdrop-blur-sm border border-navy-700/50">
                ...
                <div className="text-3xl font-bold text-gold-400">{formSettings?.fee || "Rs. 0"}</div>
              </div>
                ...
                {paymentMethod === 'upi' ? (
                  ...
                  >
                    <div className="bg-white p-6 rounded-2xl flex flex-col items-center justify-center">
                      ...
                      <p className="text-navy-400 text-xs mt-4 font-medium uppercase tracking-widest">Scan to Pay</p>
                    </div>
                    
                    <div>
                      <label className="text-xs font-bold uppercase tracking-widest text-navy-300 mb-2 block">UPI ID</label>
                      <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => handleCopy(formSettings?.payment?.upiId || "example@upi")}
                  ...
                  >
                    <div className="bg-navy-950 p-6 rounded-2xl space-y-4 border border-navy-800">
                      <div>
                        <div className="text-[10px] uppercase font-bold text-navy-400 mb-1">Account Name</div>
                        <div className="font-medium text-white">{formSettings?.payment?.bankDetails?.accountName || "Name"}</div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase font-bold text-navy-400 mb-1">Account Number</div>
                        <div className="font-mono text-gold-400">{formSettings?.payment?.bankDetails?.accountNumber || "123456789"}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <div className="text-[10px] uppercase font-bold text-navy-400 mb-1">IFSC Code</div>
                          <div className="font-mono text-white">{formSettings?.payment?.bankDetails?.ifsc || "IFSC"}</div>
                        </div>
                        <div>
                          <div className="text-[10px] uppercase font-bold text-navy-400 mb-1">Bank Name</div>
                          <div className="font-medium text-white">{formSettings?.payment?.bankDetails?.bankName || "Bank"}</div>
                        </div>
                      </div>
                    </div>
                  </m.div>
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-3xl shadow-premium border border-slate-100 overflow-hidden">
                <div className="p-8 border-b border-slate-100 bg-slate-50/50">
                    ...
                    <p className="text-slate-500 mt-2 text-sm leading-relaxed">{config.description}</p>
                </div>
                <DynamicForm
            ...
            <div className="p-12 text-center bg-white rounded-3xl shadow-premium border border-slate-100">
              ...
              <p className="text-slate-500 text-sm">The form &apos;{slug}&apos; could not be loaded. It may have been disabled or deleted.</p>
            </div>
          )}
        </m.div>
      </div>
