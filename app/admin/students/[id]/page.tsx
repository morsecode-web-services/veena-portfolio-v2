...
export default function StudentDetailPage() {
  ...
  const handleToggleTelegramJoin = async (enrollmentId: string, currentJoined: boolean) => {
    ...
    try {
      ...
      addToast(
        currentJoined ? 'Marked Telegram as Left.' : 'Marked Telegram as Joined!',
        'success'
      );
      fetchData();
    } catch (err: any) {
                  ...
                  <div key={enrollment.id} className="p-5 hover:bg-slate-50/30 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-slate-800 text-sm">
                            {enrollment.cohorts?.title || 'Unknown Cohort'}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border ${getStatusStyles(enrollment.status)}`}
                          >
                            {enrollment.status}
                          </span>
                        </div>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-slate-400 font-medium">
                          <span className="flex items-center gap-1">
                            <Clock size={10} /> {formatDateSafe(enrollment.created_at)}
                          </span>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 self-start">
                        <span
                          className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider border flex items-center gap-1 ${
                        ...
                        <div className="flex items-center gap-0.5 bg-slate-50 border border-slate-200 rounded p-0.5">
                          <button
                            type="button"
                            onClick={() =>
                              handleToggleTelegramJoin(enrollment.id, enrollment.telegram_joined)
                            }
                          ...
                          {!enrollment.telegram_joined && enrollment.telegram_invite_link && (
                            <>
                              <button
                                ...
                                onClick={() =>
                                  copyToClipboard(
                                    enrollment.telegram_invite_link!,
                                    'link_' + enrollment.id
                                  )
                                }
                                className={`w-7 h-7 rounded flex items-center justify-center transition-all ${
                                  copiedId === 'link_' + enrollment.id
                    ...
                    {payments.map((payment) => (
                      <tr key={payment.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-5 py-3 space-y-0.5">
                          <div
                            className="font-bold text-slate-800 truncate max-w-[160px] font-mono text-[10px]"
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border ${getStatusStyles(payment.status)}`}
                          >
                            {payment.status}
                          </span>
                        </td>
            ...
            <div className="p-4 space-y-3">
              {invitations.length === 0 ? (
                <p className="text-xs text-slate-400 italic text-center py-3">
                  No re-enrollment invitations sent yet.
                </p>
              ) : (
                invitations.map((invite) => (
                  <div
                    key={invite.id}
                    className="pb-3 last:pb-0 border-b border-slate-100 last:border-b-0 space-y-1.5"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-semibold text-slate-800 truncate">
                        {invite.cohorts?.title}
                      </span>
                      <span
                        className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider border ${getStatusStyles(invite.status)}`}
                      >
                        {invite.status}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                      ...
                      {invite.payment_link_url && (
                        <button
                          onClick={() =>
                            copyToClipboard(invite.payment_link_url!, 'invite_' + invite.id)
                          }
                  ...
                  {logs.map((log) => {
                    ...
                    return (
                      <div key={log.id} className="relative space-y-0.5">
                        <div
                          className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 bg-white ${dotColor}`}
                        />
                        <div className="flex items-center justify-between gap-2">
