...
export default function SmartLinksPage() {
  ...
  const handleResetClicks = async (id: string) => {
    ...
    const { error } = await supabase.from('smart_links').update({ clicks: 0 }).eq('id', id);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">
            Create short, trackable redirect links. Optionally enable deep linking to open native
            apps like YouTube and Instagram on mobile.
          </p>
        </div>
        <button
          onClick={fetchLinks}
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-5">
        <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-4 flex items-center gap-1.5 border-b border-slate-100 pb-2">
          {editingId ? (
            ...
            <Plus className="w-3.5 h-3.5 text-slate-505" />
          )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-1 italic">
                Optional. Only used if displayed on the Link-in-Bio page.
              </p>
            </div>
            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
              ...
              <div className="flex items-center h-8">
                <label className="flex items-center cursor-pointer relative">
                  ...
                  />
                  <div
                    className={`w-9 h-5 rounded-full transition duration-200 ease-in-out ${showInBio ? 'bg-slate-900' : 'bg-slate-300'}`}
                  ></div>
                  <div
                    className={`absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition-transform duration-200 ease-in-out ${showInBio ? 'transform translate-x-4' : ''}`}
                  ></div>
                </label>
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1 italic">
              {platform === 'other'
                ? 'Redirects straight to the target URL on all devices.'
                : 'On mobile, attempts to open the native app before falling back to the web URL.'}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center text-slate-400 bg-slate-50/50">
              ...
              <p className="text-xs font-semibold text-slate-850">
                No smart links created yet. Create your first one above!
              </p>
            </div>
          ) : (
                ...
                {links.map((link) => (
                  <tr key={link.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-5 py-3">
                      ...
                      <div
                        className="text-[10px] text-slate-400 mt-0.5 truncate max-w-[150px] sm:max-w-xs xl:max-w-md"
