  ...
  return (
    <div className="bg-white rounded border border-slate-200 p-4">
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="p-1.5 rounded" style={{ background: `${color}15`, color }}>
          {icon}
        </div>
      </div>
      ...
      <div className="mt-2 flex items-center gap-1">
        ...
        <span className={`text-[11px] font-bold ${pos ? 'text-emerald-500' : 'text-red-400'}`}>
          {change > 0 ? '+' : ''}
          {change}%
        </span>
...
export default function AnalyticsPage() {
  ...
  const fetchCohortStats = useCallback(async () => {
    ...
    if (cohorts) {
      ...
      const stats = cohorts.map((c) => {
        ...
        // Lead/Interest count
        const leadCount =
          (leads?.filter((l) => l.cohort_id === c.id).length || 0) +
  ...
  return (
    <div className="max-w-[1400px] mx-auto pb-20 relative">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded">
          <div className="flex flex-col items-center gap-2 bg-white rounded shadow-sm px-6 py-4 border border-slate-200">
            ...
            <p className="text-xs font-bold text-slate-800">
              Loading {getRangeLabel(dateRange).toLowerCase()} data…
            </p>
          </div>
        </div>
      )}
      ...
      <div
        className={`space-y-5 transition-opacity duration-200 ${loading ? 'opacity-40 pointer-events-none' : ''}`}
      >
        {/* Header */}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {today && (
              <div className="flex items-center gap-1 bg-emerald-50 text-emerald-700 px-2.5 py-1.5 rounded border border-emerald-250 text-xs font-semibold">
                ...
                {today.users} users today · {today.pageViews} views
              </div>
            )}
            <button
              onClick={() => fetch_(true)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <KPI
            ...
            change={overview.pageViews.change}
            icon={<Eye className="w-4 h-4" />}
            color={C.green}
          />
          <KPI
            title="Sessions"
            value={overview.sessions.value}
          ...
          <KPI
            title="Avg Duration"
            value={dur(overview.avgDuration.value)}
            change={overview.avgDuration.change}
          ...
          <KPI
            ...
            change={overview.bounceRate.change}
        ...
        <div className="bg-white rounded border border-slate-200 p-5">
          <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-4">
            Traffic Trend · {getRangeLabel(dateRange)}
          </h2>
          <div className="h-64">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2">
            {data.funnel ? (
              <Section
                title="Cohort Enrollment Funnel"
                icon={<Zap className="w-4 h-4 text-slate-450" />}
              >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                  {[
                    {
                      label: 'Cohort Views',
                      value: data.funnel.view_item,
                      color: C.blue,
                      sub: 'Initial Interest',
                    },
                    {
                      label: 'Form Opens',
                      value: data.funnel.begin_checkout,
                      color: C.purple,
                      sub: 'Purchase Intent',
                    },
                    {
                      ...
                      color: C.green,
                      sub: 'Revenue Realized',
                    },
                  ].map((step, i, arr) => {
                    const prev = arr[i - 1];
                    const conversion = prev
                      ? Math.round((step.value / prev.value) * 100) || 0
                      : 100;
          ...
          <div className="lg:col-span-1">
            <Section
              title="Cohort Revenue & Enrollment"
                ...
                <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
                  {cohortStats.length > 0 ? (
                    cohortStats.map((c: any, i: number) => (
                      <div
                        key={i}
                        className="p-2.5 rounded border border-slate-200 bg-slate-50/30 hover:bg-white transition-all"
                      >
                        <div className="flex items-center justify-between mb-1">
                          <div
                            className="font-bold text-[11px] text-slate-800 truncate max-w-[140px]"
                            title={c.title}
                          >
                            {c.title}
                          </div>
                          <span
                            className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold uppercase tracking-wider ${
                              c.pricing_type === 'pay_as_you_wish'
                        ...
                        <div className="flex items-center justify-between text-[10px] text-slate-500 font-medium">
                          <span>
                            {c.student_count} Paid{' '}
                            {c.lead_count > 0 && (
                              <span className="text-slate-400">({c.lead_count} Leads)</span>
                            )}
                          </span>
                  ...
                  {topPages?.map((p: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-50/50">
                      <td className="py-2 pr-2">
                        <div
                          className="font-medium text-slate-900 truncate max-w-[160px]"
                          title={p.title}
                        >
                          {p.title || p.path}
                        </div>
                      ...
                      <td className="py-2 text-right font-bold text-slate-850">
                      ...
                      <td
                        className={`py-2 text-right font-medium ${p.bounceRate > 70 ? 'text-red-500' : p.bounceRate > 50 ? 'text-orange-500' : 'text-emerald-500'}`}
                      >
                        {p.bounceRate}%
                      </td>
                    </tr>
                    ...
                    <tr key={i} className="hover:bg-slate-50/50">
                      <td className="py-2 pr-2">
                        ...
                        <div className="text-slate-400">
                          {s.source !== '(direct)' && s.source !== s.channel ? s.source : ''}
                        </div>
                      </td>
                      <td className="py-2 text-right font-bold text-slate-850">
                      ...
                      <td
                        className={`py-2 text-right font-medium ${s.bounceRate > 70 ? 'text-red-500' : s.bounceRate > 50 ? 'text-orange-500' : 'text-emerald-500'}`}
                      >
                        {s.bounceRate}%
                      </td>
                    </tr>
              ...
              return (
                <div key={d.device} className="flex items-center justify-between text-xs mb-1.5">
                  ...
                  <span className="font-bold">
                    {pct}% <span className="text-gray-400 font-normal">({d.users})</span>
                  </span>
                </div>
              );
            })}
                ...
                <div key={o.os}>
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded">
                    <div
                      ...
                      style={{
                        width: `${Math.round((o.users / maxOS) * 100)}%`,
                ...
                <div key={b.browser}>
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded">
                    <div
                      className="h-full rounded"
                      style={{
                        width: `${Math.round((b.sessions / (browsers[0]?.sessions || 1)) * 100)}%`,
                        background: C.purple,
                      }}
                    />
                  </div>
                ...
                <div key={e.event}>
                  ...
                  <div className="w-full h-1.5 bg-slate-100 rounded">
                    <div
                      className="h-full rounded"
                      style={{
                        width: `${Math.round((e.count / maxEvt) * 100)}%`,
                        background: C.pink,
                      }}
                    />
                  </div>
        ...
        {/* Heatmap */}
        <Section
          title="Activity Heatmap · Day × Hour (Local Time)"
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[600px]">
              ...
              {DAYS.map((day, di) => (
                <div
                  key={day}
                  className="grid grid-cols-[44px_repeat(24,1fr)] gap-[3px] mb-[3px] items-center"
                >
                  <div className="text-[10px] text-gray-500 font-medium pr-1 text-right">
                    {day.substring(0, 3)}
                  </div>
                  {HOURS.map((h) => {
              ...
              <div className="flex items-center justify-end gap-2 mt-3">
                ...
                {[0.06, 0.2, 0.4, 0.65, 0.9].map((o) => (
                  <div
                    ...
                    style={{ background: C.orange, opacity: o }}
                  />
                ))}
