...
export function TemplateBuilder({ cohortId }: { cohortId: string }) {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        setFieldsConfig(prev => prev.map(f => {
            if (f.id === draggingId) {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 pb-20 p-6" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
            {/* Header */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-4">
                    <button onClick={() => window.history.back()} className="p-2 bg-slate-100 hover:bg-slate-200 rounded text-slate-600 transition-colors">
                        <ArrowLeft size={16} />
                    </button>
                    <div>
                        <h1 className="text-xl font-bold text-slate-900">Certificate Template Builder</h1>
                        <p className="text-slate-500 text-xs mt-0.5">Cohort: <span className="font-bold text-slate-700">{cohortName}</span></p>
                    </div>
                </div>
                <button 
                    onClick={handleSave}
                    ...
                    {selectedField && (
                        <div className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
                            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                                ...
                                <button onClick={() => removeField(selectedField.id)} className="text-red-500 hover:text-red-600 transition-colors" title="Delete Element">
                                    <Trash2 size={14} />
                                </button>
                            </div>
                            ...
                            {selectedField.type === 'static' && (
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Text Content</label>
                                    <textarea 
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Font Family</label>
                                <select 
                                    value={selectedField.fontFamily}
                                    onChange={(e) => updateField(selectedField.id, { fontFamily: e.target.value })}
                                    className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded text-xs outline-none focus:border-slate-800 transition-all"
                                >
                                    {DEFAULT_FONT_FAMILIES.map(ff => <option key={ff} value={ff}>{ff}</option>)}
                                </select>
                            </div>

                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Text Align</label>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between">
                                <span className="text-[9px] font-bold text-slate-400 uppercase">Pos X: {selectedField.x}</span>
                                <span className="text-[9px] font-bold text-slate-400 uppercase">Pos Y: {selectedField.y}</span>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    >
                        ...
                        {fieldsConfig.map(field => {
                            ...
                            if (field.type === 'dynamic') {
                                ...
                                if (field.id === 'learning_outcomes') displayText = "• Outcome 1\n• Outcome 2\n• Outcome 3";
                            }
