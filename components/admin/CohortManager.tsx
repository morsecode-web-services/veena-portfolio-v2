...
export function CohortManager() {
  ...
  const [reenrollProgress, setReenrollProgress] = useState({
    current: 0,
    total: 0,
    success: 0,
    failed: 0,
  ...
  const openCertificateModal = async (cohort: Cohort) => {
    ...
    try {
      const { data: enrollments, error } = await supabase
        .from('enrollments')
        .select(
          'id, student_id, students(name, email, phone), telegram_joined, certificate_sent_at'
        )
    ...
    try {
      ...
      for (let i = 0; i < studentIds.length; i += BATCH_SIZE) {
        ...
        const response = await fetch(
          `/api/admin/cohorts/${certificateCohort.id}/send-certificates`,
          {
            method: 'POST',
            headers: {
  ...
  const handleReenrollBatch = async () => {
    ...
    try {
      ...
      if (totalSuccess > 0) {
        if (totalFailed > 0) {
          addToast(
            `Sent ${totalSuccess} invitations successfully, but ${totalFailed} failed.`,
            'info'
          );
        } else if (totalSkipped > 0) {
          addToast(
            `Sent ${totalSuccess} invitations successfully (${totalSkipped} already invited/enrolled).`,
            'success'
          );
        } else {
        ...
        addToast(
          `No invitations sent. All ${totalSkipped} students were skipped (already invited or enrolled).`,
          'info'
        );
      } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        learning_outcomes: (formData.learning_outcomes_raw || '')
          .split('\n')
          .filter((s: string) => s.trim()),
        curriculum_highlights: (formData.curriculum_highlights_raw || '')
          .split('\n')
          .filter((s: string) => s.trim()),
      ...
      if (editingId) {
        const { error } = await supabase.from('cohorts').update(payload).eq('id', editingId);
        if (error) throw error;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">
            Manage your monthly learning batches and pricing.
          </p>
        </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <section className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                        SHORT DESCRIPTION (Shows on card & modal)
                      </label>
                      <textarea
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <p className="mt-1 text-[10px] text-slate-450 italic">
                        If set to Pay As You Wish, the suggestions price acts as template
                        placeholder contribution at checkout, but students can contribute any custom
                        amount (min ₹1).
                      </p>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                        CURRENT PRICE / SUGGESTED AMOUNT (₹)
                      </label>
                      <input
                        type="number"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            original_price: Math.round(parseFloat(e.target.value) * 100) || 0,
                          })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, razorpay_plan_id: e.target.value })
                        }
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, learning_outcomes_raw: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, curriculum_highlights_raw: e.target.value })
                        }
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded text-xs outline-none focus:border-slate-800 focus:ring-1 focus:ring-slate-900 transition-all min-h-[100px] text-slate-800"
                ...
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <div
                        className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${formData.is_highlighted ? 'right-0.5' : 'left-0.5'}`}
                      />
                    </button>
                  </div>
                  <div className="flex items-center justify-between">
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
          ...
          <button
            ...
            className="p-1.5 hover:bg-white rounded border border-transparent hover:border-slate-200 transition-colors"
          >
                ...
                <tr key={cohort.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-col">
                      {cohort.pricing_type === 'pay_as_you_wish' ? (
                        <>
                          <span className="text-[8px] font-bold uppercase tracking-wider text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100 w-max mb-0.5">
                            Pay As You Wish
                          </span>
                          <span className="text-[10px] text-slate-400">
                            Suggested: ₹{(cohort.price / 100).toFixed(0)}
                          </span>
                        </>
                      ) : (
                        <>
                          <span className="font-bold text-slate-800">
                            ₹{(cohort.price / 100).toFixed(0)}
                          </span>
                          {cohort.original_price && (
                            <span className="text-[9px] text-slate-300 line-through">
                              ₹{(cohort.original_price / 100).toFixed(0)}
                            </span>
                          )}
                        </>
                      )}
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      {cohort.invite_conversion && cohort.invite_conversion.total > 0 && (
                        <div
                          ...
                          title="Re-enrollment conversion: (Paid / Sent)"
                        >
                          Re-enroll:{' '}
                          <span className="font-bold text-slate-700">
                            {cohort.invite_conversion.paid}
                          </span>
                          /{cohort.invite_conversion.total} (
                          {Math.round(
                            (cohort.invite_conversion.paid / cohort.invite_conversion.total) * 100
                          )}
                          %)
                        </div>
                      )}
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex justify-end gap-1.5">
                      ...
                      <button
                        onClick={() =>
                          (window.location.href = `/admin/cohorts/${cohort.id}/template-builder`)
                        }
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div>
                ...
                >
                  ...
                  {cohorts
                    .filter((c) => c.id !== reenrollTargetId)
                    .map((c) => (
                      <option key={c.id} value={c.id}>
                        {c.month_name} - {c.title}
                      </option>
                    ))}
                </select>
                <p className="mt-1.5 text-[10px] text-slate-400 leading-relaxed italic">
                  We will find all successful enrollees from the selected batch and generate unique
                  payment links for them.
                </p>
              </div>
                  ...
                  {sourceStudentsLoading ? (
                    ...
                    <div className="text-center py-4 text-slate-400 bg-slate-50/50 rounded border border-slate-200">
                      <p className="text-xs font-bold">
                        No eligible paid students found in this cohort
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
                        ...
                        <span className="font-bold text-slate-700">
                          {selectedStudentEmails.size} of {sourceStudents.length} selected
                        </span>
                      </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider">
                    <span className="text-slate-800">
                      Progress: {reenrollProgress.current} / {reenrollProgress.total}
                    </span>
                    <span className="text-slate-500">
                      {Math.round((reenrollProgress.current / reenrollProgress.total) * 100)}%
                    </span>
                  </div>
                  <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(reenrollProgress.current / reenrollProgress.total) * 100}%`,
                      }}
                    />
                  </div>
                  <div className="flex gap-3 text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                    <span>
                      Sent: <span className="text-green-600">{reenrollProgress.success}</span>
                    </span>
                    <span>
                      Skipped: <span className="text-slate-600">{reenrollProgress.skipped}</span>
                    </span>
                    <span>
                      Failed: <span className="text-red-500">{reenrollProgress.failed}</span>
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md overflow-hidden flex flex-col max-h-[90vh] border border-slate-200">
            ...
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 text-sm">
                  Send Participation Certificates
                </h3>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div>
                ...
                <p className="mt-1.5 text-[10px] text-slate-400 leading-relaxed italic">
                  Only active students who have joined the Telegram group and haven&apos;t received
                  a certificate yet are shown below.
                </p>
              </div>
                ...
                {certLoading ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
                      ...
                      <span className="font-bold text-slate-700">
                        {selectedCertStudents.size} of {eligibleStudents.length} selected
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          const filtered = eligibleStudents.filter((s) =>
                            s.name.toLowerCase().includes(certSearchQuery.toLowerCase())
                          );
                          const allSelected = filtered.every((s) =>
                            selectedCertStudents.has(s.student_id)
                          );
                      ...
                      >
                        {eligibleStudents
                          ...
                          .every((s) => selectedCertStudents.has(s.student_id))
                          ? 'Deselect All'
                          : 'Select All'}
                      </button>
                    </div>
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded p-3 flex items-start gap-2">
                ...
                <p className="text-[10px] text-emerald-800 leading-normal">
                  Certificates will be generated dynamically and sent directly to the students via
                  WhatsApp.
                </p>
              </div>
            </div>
            ...
            <div className="px-5 py-4 bg-slate-50 border-t border-slate-200 flex gap-3">
              ...
              <button
                ...
                disabled={
                  certLoading || eligibleStudents.length === 0 || selectedCertStudents.size === 0
                }
