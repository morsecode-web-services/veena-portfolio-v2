...
export function VideoManager() {
  ...
  const handleUpdateVideo = async (id: string) => {
    try {
      const { error } = await supabase.from('videos').update(editData).eq('id', id);
  ...
  if (loading || !siteConfig)
    return (
      <div className="flex items-center justify-center h-64 text-gray-500">
        ...
        {loading ? 'Loading videos...' : 'Finalizing configuration...'}
      </div>
    );
      ...
      <div className="bg-white rounded border border-slate-200 shadow-none overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
          <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
            All Performances ({videos.length})
          </h3>
          <button
            onClick={fetchVideos}
            className="p-1.5 hover:bg-slate-100 rounded transition-colors text-slate-400 hover:text-slate-650"
          >
            ...
            <tbody className="divide-y divide-slate-200">
                    ...
                    {editingId === video.id ? (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const subcatId =
                            siteConfig.music.categories.find((c) => c.id === catId)
                              ?.subcategories[0]?.id || '';
                          setEditData({
                            ...editData,
                            category_id: catId,
                  ...
                  <td className="px-4 py-3 text-xs font-medium text-slate-500 capitalize">
                    {editingId === video.id ? (
                      <select
                        ...
                        onChange={(e) =>
                          setEditData({ ...editData, subcategory_id: e.target.value })
                        }
                        className="w-full px-2 py-1 text-xs border border-slate-350 rounded focus:outline-none focus:ring-1 focus:ring-slate-900 bg-white"
                      >
              ...
              {videos.length === 0 && !loading && (
                <tr>
                  <td
                    colSpan={6}
                    className="px-4 py-12 text-center text-slate-400 font-medium italic text-xs"
                  >
