    ...
    <div className={`phone-input-wrapper ${error ? 'has-error' : ''}`}>
      <Controller
        ...
        render={({ field: { onChange, value } }) => (
          <div
            className={`flex items-center w-full rounded-xl border transition-all bg-white overflow-hidden ${
              error
          ...
          >
            ...
            <div className="flex items-center gap-2 pl-3 md:pl-4 pr-2 md:pr-3 py-3 border-r border-slate-200 bg-slate-50/50 hover:bg-slate-100 transition-colors cursor-pointer group relative">
              <div className="flex items-center gap-1.5 pointer-events-none mt-0.5">
                <span className="text-sm md:text-base leading-none">{getFlagEmoji(country)}</span>
                <span className="text-[13px] md:text-sm font-bold text-navy-900 leading-none">
...
export default function DynamicForm({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
        ...
        const options = {
          ...
          handler: async function (response: any) {
            try {
              ...
              setErrorMessage(
                'Payment succeeded, but we failed to save your submission. Please contact support.'
              );
            } finally {
      ...
      if (
        ...
        error.message?.includes('valid mobile number')
      ) {
        console.warn('Validation check:', error.message);
      } else {
        // Using warn instead of error to prevent Next.js dev overlay pill from popping up for user-facing API errors
        console.warn('Form submission message:', error.message);
      }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
    ...
    if (!uploadPreset || !cloudName) {
      alert(
        'Cloudinary upload preset or cloud name is missing. Please configure NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET.'
      );
      return;
    }
    ...
    return (
      <div className="bg-white p-8 rounded-2xl shadow-premium border border-slate-100 text-center">
        ...
        <p className="text-slate-600 mb-6">
          {successMessage || 'Thank you for reaching out. Aishwarya will get back to you shortly.'}
        </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {fields.map((field) => (
          <div key={field.name}>
            {field.type === 'content' ? (
              <div
                className="prose prose-sm md:prose-base prose-navy max-w-none my-4"
                dangerouslySetInnerHTML={{ __html: field.content || field.label || '' }}
              />
            ) : (
              <>
                <label
                  htmlFor={`${formSlug}-${field.name}`}
                  className="block text-[10px] md:text-[11px] font-bold text-navy-700 uppercase tracking-wider mb-1.5"
                >
                ...
                {field.type === 'textarea' ? (
                  <textarea
                    ...
                    className={`w-full px-4 py-3 rounded-xl border transition-all text-[13px] md:text-sm font-medium focus:ring-2 focus:ring-navy-500/10 focus:border-navy-500 outline-none ${
                      errors[field.name]
                        ? 'border-red-500 bg-red-50/30'
                  ...
                  <select
                    id={`${formSlug}-${field.name}`}
                    {...register(field.name)}
                    className={`w-full px-4 py-3 rounded-xl border transition-all text-[13px] md:text-sm font-medium focus:ring-2 focus:ring-navy-500/10 focus:border-navy-500 outline-none appearance-none bg-no-repeat bg-[length:16px_16px] bg-[right_1.5rem_center] cursor-pointer ${
                      errors[field.name]
                        ? 'border-red-500 bg-red-50/30'
                        : 'border-slate-200 hover:border-slate-300'
                    }`}
                  ...
                  <div className="mt-1">
                    {watch(field.name) ? (
                      ...
                      <div className="relative">
                        ...
                        />
                        <div
                          className={`w-full px-4 py-8 rounded-xl border-2 border-dashed flex flex-col items-center justify-center transition-all text-center
                  ...
                  <input
                    ...
                    {...register(field.name)}
                    placeholder={field.placeholder}
                    className={`w-full px-3 md:px-4 py-3 rounded-xl border transition-all text-[13px] md:text-sm font-medium focus:ring-2 focus:ring-navy-500/10 focus:border-navy-500 outline-none ${
                      errors[field.name]
                        ? 'border-red-500 bg-red-50/30'
                        : 'border-slate-200 hover:border-slate-300'
                    }`}
