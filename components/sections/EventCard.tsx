...
import { MapPin, Clock, ExternalLink, ArrowRight, X, Maximize2 } from 'lucide-react';
...
export const EventCard: React.FC<EventCardProps> = ({ event, isPast, viewMode = 'grid' }) => {
  ...
  if (viewMode === 'timeline') {
    return (
      <>
        <m.div variants={itemVariants} className="group flex gap-4 sm:gap-6">
          {/* Minimal Date Column */}
          <div className="w-16 sm:w-20 pt-1 text-right flex-shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">
            ...
            <span className="block text-xs text-navy-400 font-medium uppercase tracking-wider">
              {dateInfo?.weekday}
            </span>
          </div>
          ...
          <div className="pb-8 pt-0.5 flex-grow">
            ...
            {hasMap && (
              <a
                ...
                onClick={() =>
                  analytics.externalLinkClick('Event Details Map', event.map_url!, 'map')
                }
        ...
        <AnimatePresence>
          {isLightboxOpen && hasImage && (
            ...
            >
              <LightboxContent event={event} onClose={() => setIsLightboxOpen(false)} />
            </m.div>
          )}
        </AnimatePresence>
      </>
  ...
  return (
    <>
      <m.div variants={itemVariants} className="group w-full max-w-sm">
        <Card
          variant="outlined"
          padding="none"
          ...
          {hasImage && (
            ...
            >
              ...
              <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-3 py-2 rounded-lg shadow-sm border border-navy-50 text-center min-w-[50px]">
                <span className="block text-xs font-black text-navy-400 uppercase leading-none mb-1">
                  {dateInfo?.month}
                </span>
                <span className="block text-xl font-serif font-bold text-navy-900 leading-none">
                  {dateInfo?.day}
                </span>
              </div>
            </div>
              ...
              <div className="mt-2 pt-4 flex items-center justify-between">
                {hasBooking && !isPast ? (
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() =>
                      analytics.externalLinkClick('Event Tickets', event.booking_url!, 'tickets')
                    }
                ...
                {hasMap && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() =>
                      analytics.externalLinkClick('Event Map Icon', event.map_url!, 'map')
                    }
